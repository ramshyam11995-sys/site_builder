...
const Myprojects = () => {
  ...
  const deleteProject = async (_projectId: string) => {
    try {
      ...
      if (!confirm) return;
      const { data } = await api.delete(`/api/project/${_projectId}`);
        ...
        {loading ? (
          ...
          <div className='py-10 min-h[80vh]' >
            <div className='flex item-center justify-between mb-12'>
              ...
              <button onClick={() => navigate('/')} className='flex item-center gap-2 text-white px-3
            sm:px-6 py-1 sm:py-2 rounded bg-linear-to-br from-indigo-500
            to-indigo-600 hover:opacity-90 active:scale-95 transition-all'>
            ...
            <div className='flex flex-wrap gap-3.5'>
              ...
              max-sm:mx-auto cursor-pointer bg-gray-900/60 border border-gray-700 rounded-lg overflow-hidden shadow-md group hover:shadow-indido-700/30 hover:border-indigo-800/80 transition-all duration-300'>
                  {/*Destop-like Mini Preview*/}
                ...
                overflow-hidden border-b border-gray-800'>
                    {project.current_code ? (
                      ...
                      <iframe
                        ...
                        className='absolute top-0 left-0 w-[1200px] h-[800px] origin-top-left pointer-events-none' sandbox='allow-scripts allow-same-origin'
          ...
          bg-indigo-500 hover:bg-indigo-600 active:scale-95 transition-all'>Create New</button>
          </div>
        )}
