...
`,
        ...
        if (!code) {
            ...
            return res.status(500).json({
                message: "Unable to generate the code, please try again",
            });
        }
        ...
        // Preserve your existing credit refund behavior.
        if (userId) {
            try {
                await prisma.user.update({
                    where: { id: userId },
                ...
                console.error(
                    ...
                    refundError.code || refundError.message
                );
            }
        }
    ...
    try {
        ...
        if (!projectId || !versionId) {
            return res.status(400).json({
                message: "Invalid project or version id",
            });
        }
...
export const getProjectPreview = async (
    ...
    try {
        ...
        const project = await prisma.websiteProject.findUnique({
            ...
            include: {
                // IMPORTANT:
                // Prisma schema uses "conversation", not "conversations"
                conversation: {
                    orderBy: {
                        timestamp: "asc",
                    },
